...
class HyStats(commands.Cog):
    ...
    async def profiles(self, ctx, user):
        ...
        ctx = self.bot.get_channel(ct.channel.id)
        uuid = self.util.toUUID(name)
        if uuid is False:
        ...
        if d.status_code == 429 or d.status_code == 521:
            await ctx.send(embed=discord.Embed(title="API Error", description=self.util.handleRequest(d.status_code), color=discord.Color.red()), components=[Button(label="Remove",style=4)])
            return
        f = d.json()
            ...
            for name, member in pf['members'].items():
                if "coop_invitation" in member:
                    if member["coop_invitation"]["confirmed"] == False:
                        coopm.append({"name":self.util.toName(name), "title":"Invited Coop Member"})
                        continue
                    isCoop = True
                    coopm.append({"name":self.util.toName(name), "title":"Coop Member"})
                else:
                    if isCoop2: isCoop = True
                    coopm.append({"name":self.util.toName(name), "title":"Coop Owner",}) 
                    isCoop2 = True
            if isCoop == False:
        ...
        e = discord.Embed(title=f"{self.util.toName(uuid)} on {pf['cute_name']}", color=discord.Color.green())
    ...
    async def counts(self, ctx, type='SKYBLOCK'):
        await ctx.defer()
